          ...
          <a-form-item label="所属公司" :labelCol="labelCol" :wrapperCol="wrapperCol">
            ...
            >
              <a-select-option
                  ...
                  :key="item.id"
                  :value="item.id+''">{{item.companyFullName}}
              </a-select-option>
            </a-select>
          </a-form-item>
        </a-col>
        ...
        <a-col :span="12">
          <a-form-item label="选择住宅" :labelCol="labelCol" :wrapperCol="wrapperCol">
            <a-select
              ...
              <a-select-option v-for="item in estateSelect" :key="item.estateCode" :value="item.estateCode">{{ item.estateName }}</a-select-option>
            </a-select>
          </a-form-item>
        </a-col>
        <a-col :span="12">
...
<script>
import { selectCompany, selectEstateByCompany } from '@/api/estate'
...
export default {
    ...
    methods: {
        ...
        changeCompay(val) {
            console.log(val)
            selectEstateByCompany({ company: val }).then(res => {
                // console.log(res)
                this.estateSelect = res.result
            })
        },
        changeEstate() {
            console.log('changeEstate')
        }
    }
}
</script>
